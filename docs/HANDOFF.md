...
- **P1.7 Blockregel vollstaendig.** `blocks` weist je Manager alle drei Bedingungen **einzeln** aus (`conditions.blockPointsBelowLine`, `.seasonPointsBelowLine`, `.stageCapableUnderHalf` mit Wert, Linie und `failed`), dazu `strikeEarned` als Und-Verknuepfung. Seit 16.09. traegt jede Bedingung zusaetzlich `gap` (Abstand zur Vorgabe, positiv = Puffer, negativ = fehlt), die dritte ausserdem `needed` (kleinste ganze Rundenzahl, die die halbe Blocklaenge erfuellt: 7 Runden -> 4). Grund: Ohne die Differenz sieht ein Manager nur bestanden/nicht bestanden und kann nicht einschaetzen, wie weit er von einem Strike entfernt ist. **Ebenfalls seit 16.09. (Entscheidung Captain):** Im ERSTEN Block eines Sets ist der Saisonwert **Blockpunkte + Endstand des Vorsets** (`conditions.seasonPointsBelowLine.source = block_plus_previous_set`, dazu `previousSetPoints`; Blockfelder `seasonSource`/`seasonSourceSetId`). Vorher war der Saisonwert im ersten Block mit dem Blockwert identisch, die zweite Bedingung also eine Kopie der ersten. **Ab Block 2 faellt das Vorset raus**, dann zaehlt allein der laufende Saisonstand des Sets. Manager ohne Eintrag im Vorset (Neuzugang) haben `value: null` und reissen die Bedingung nicht. Block 1 von Hyperglitch nach 7 Runden: Ø **206,3**, Linie **165,04**; nur enexxx liegt darunter (143), McBeast (187) und Sorare | MA (182) sind damit KEINE Strike-Kandidaten mehr. Stage-faehig = eigener Score >= `round.target / 3`. Gegenprobe Set 1: Saison-Ø **167,3**, Linie **133,8** - exakt die Werte aus der Strike-Begruendung fuer enexxx.
...
weiter (jr3hr -> r3hr-7625d620-...), `so5Fixture.userFixtureResults(userSlug:)` dagegen NICHT
("User not found"). Jede Abfrage mit `userSlug:`-Argument deshalb bei "not found" ueber
`user(slug:){slug}` aufloesen (BUG-044).
