...
0g. 🔄 **IN ARBEIT (01.08.) — Gespeicherte Portfolios statt Live-Abruf.** Ziel: API-Verbrauch senken, BEVOR ein zweiter Sorare-Key beantragt wird. Heute kostet jeder Portfolio-Aufruf 6–16 Sorare-Anfragen; kuenftig kommt die Anzeige aus der DB (0 Anfragen), geholt wird nur beim Sync.
   ...
   **OFFEN:** (1) Jonas: Migration ausfuehren. (2) Frontend: portfolio.html liest aus der DB statt von Sorare, Sync-Knopf, eigenes Portfolio hinter Login. (3) `delete-account` um `purge_manager_data` ergaenzen + DSGVO-Export erweitern + legal.html anpassen. (4) Danach erst zweiten Sorare-Key beantragen (Frage an Sorare: gilt das 200/min-Limit pro Key oder pro Account?).
