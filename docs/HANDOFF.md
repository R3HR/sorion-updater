...
0d. **Migration `2026-07-30_market_avg_history.sql` ausführen (30.07.):** Aggregat-RPC für die neuen Avg-FMV-7d-Chips. Hintergrund (Dev-Feedback 30.07.): Chips zeigten den Median der Einzelkarten-7d-Änderungen — anderer Wert als die Bewegung des angezeigten Durchschnitts (Avg 2,22→4,52 €, Chip -1,4 %; Rare konstant 0,0 %) → wirkte „random". UI ist schon live umgestellt (Chip = avg heute vs. ~7d zuvor aus price_history; ohne RPC bleibt der Chip leer statt falsch). Zusätzlich erledigt: Tabellen-Header 9→11px + helleres Lila, Footer index englisch, 8 Portfolio-Sortier-Tooltips englisch.
