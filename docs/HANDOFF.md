...
0h. **Sorare-Verknuepfung gebaut (01.08.) — Test durch Jonas offen:** Der Sorare-Login ist bei Sorion KEIN eigener Kontotyp, sondern der Nachweis fuer den Manager-Slug. Grund (Entscheidung Jonas): ein spaeteres Premium-Modell braucht EINE Konto-Identitaet, sonst zahlt jemand auf Konto A und nutzt Konto B. Der CraftLog-Login legt dagegen Konten unter `<slug>@sorare.craftlog.pro` an — genau dieses Zweitkonto wollen wir bei Sorion nicht.
   ...
   **Offener Test:** einmal "Mit Sorare verbinden" klicken. Kommt der Zustimmungsdialog, ist `sorion.pro/auth/callback` in der bestehenden OAuth-App registriert (die Antragsmail enthielt beide URLs) — dann brauchen wir KEINE neue App. Kommt "invalid redirect uri", muss Sorare sie ergaenzen. Von aussen nicht pruefbar: Sorare liefert fuer jede redirect_uri dieselbe SPA-Huelle aus (alle drei Testvarianten 20.058 Byte).
