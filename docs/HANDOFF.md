...
0d. **Migration `2026-07-30_market_move_v2.sql` ausführen (30.07.) — die MAßGEBLICHE, die beiden Vorgänger sind überholt:** Avg-FMV-7d-Chips. Verlauf: (a) Chip zeigte ursprünglich den Median der Einzelkarten-7d-Änderungen → anderer Wert als der angezeigte Durchschnitt (Avg 2,22→4,52 €, Chip −1,4 %; Rare konstant 0,0 %), wirkte „random" (Dev-Feedback). (b) 1. Versuch `market_avg_history` (Tagesdurchschnitte) war ebenfalls falsch — **mit Echtdaten belegt:** `price_history` erhält täglich nur die rotierende Teilmenge des Updater-Sweeps (n 537…5102) → „Tagesdurchschnitt" sprang 6,38…25,96 €; zusätzlich Timeout bei `limited`. (c) **v2 = gepaarter Korb-Vergleich** (`market_move`): nur Spieler mit Preis in BEIDEN 3-Tage-Fenstern, wertgewichtet, ≥100 Spieler nötig, plus Index `price_history(scarcity,eligibility,recorded_at)`. UI ist live umgestellt; **ohne die Migration bleiben die Chips leer** (bewusst: leer statt falsch). Nach dem Einspielen verifizieren, dass `limited` <1 s antwortet. Zusätzlich erledigt: Tabellen-Header 9→11px + helleres Lila, Footer englisch, 8 Portfolio-Sortier-Tooltips englisch.
   **Lektion:** Aggregat-Kennzahlen immer gegen Echtdaten prüfen, bevor sie in die UI gehen — `price_history` ist eine rotierende Stichprobe, kein Tages-Snapshot des Gesamtmarkts.
