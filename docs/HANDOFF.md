...
| **Kader-Abgleich** | `sync-club-rosters.mjs` | Railway (`railway-rosters.toml`), manueller Trigger — empfohlen 1x/Woche. Zieht via `football.clubsReady` (247 Clubs) + `club.activePlayers` ALLE aktiven Spieler und legt fehlende Zeilen mit `updated_at=epoch` an |
